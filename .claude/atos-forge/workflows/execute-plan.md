...
@/Users/igor.3.wolak.external/Downloads/.claude/atos-forge/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs init execute-phase "${PHASE}" --include state,config)
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="forge-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn forge-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs phases list --type summaries --raw
...
See `/Users/igor.3.wolak.external/Downloads/.claude/atos-forge/references/tdd.md` for structure.
</tdd_plan_execution>
...
See /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `/Users/igor.3.wolak.external/Downloads/.claude/atos-forge/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `/Users/igor.3.wolak.external/Downloads/.claude/atos-forge/templates/summary.md`.
...
Update STATE.md using forge-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs state advance-plan

# Recalculate progress bar from disk state
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs state update-progress

# Record execution metrics
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs state record-metric \
...
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs state add-decision \
  --phase "${PHASE}" --summary "${DECISION_TEXT}" --rationale "${RATIONALE}"

# Add blockers if any found
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs state add-blocker "Blocker description"
```
</step>

<step name="update_session_continuity">
Update session info using forge-tools:
...
```bash
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs roadmap update-plan-progress "${PHASE}"
```
...
```bash
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs requirements mark-complete ${REQ_IDS}
```
...
```bash
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs commit "" --files .planning/codebase/*.md --amend
```
</step>
