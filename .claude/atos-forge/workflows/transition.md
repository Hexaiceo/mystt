...
**Delegate ROADMAP.md and STATE.md updates to forge-tools:**

```bash
TRANSITION=$(node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `forge-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs progress bar --raw)
```
...
**Use the transition result from `forge-tools phase complete`:**
...
```bash
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs config-set workflow.auto_advance false
```
