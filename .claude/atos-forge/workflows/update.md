<purpose>
Check for Forge updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether Forge is installed locally or globally by checking both locations:

```bash
# Check local first (takes priority)
# Paths templated at install time for runtime compatibility
if [ -f ./.claude/atos-forge/VERSION ]; then
  cat ./.claude/atos-forge/VERSION
  echo "LOCAL"
elif [ -f /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/VERSION ]; then
  cat /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/VERSION
  echo "GLOBAL"
else
  echo "UNKNOWN"
fi
...
```bash
npm view atos-forge version 2>/dev/null
```
...
To update manually: `node bin/install.js --global`
```
...
(Paths are relative to your install location: `/Users/igor.3.wolak.external/Downloads/.claude/` for global, `./.claude/` for local)
...
**If LOCAL install:**
```bash
rm -f ./.claude/cache/forge-update-check.json
```

**If GLOBAL install:**
```bash
rm -f /Users/igor.3.wolak.external/Downloads/.claude/cache/forge-update-check.json
```
...
Check for forge-local-patches/backup-meta.json in the config directory.
