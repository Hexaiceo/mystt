...
```bash
ROADMAP=$(node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs roadmap analyze)
```
...
2. **Run audit first** — `/forge:audit-milestone` to assess gap severity
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `forge-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to forge-tools:**

```bash
ARCHIVE=$(node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/forge:cleanup` later to archive retroactively.
...
```bash
INIT=$(node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs init execute-phase "1")
```
...
- [ ] User knows next step (/forge:new-milestone)
