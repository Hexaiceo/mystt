...
```bash
INIT=$(node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs init quick "$DESCRIPTION")
```
...
Spawn forge-executor with plan reference:
...
```bash
node /Users/igor.3.wolak.external/Downloads/.claude/atos-forge/bin/forge-tools.cjs commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
